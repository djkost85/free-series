function Rating(elementName, currentRating)
{
	...
	this.init = function()
	{
		...
		if (span)
		{
			...
			for (var i = 1; i <= 5; i++) {
				...
				star.src = 'img/icons/noRatingS.png';
				star.alt = '';
				star.rating = this;
				star.name = i;
	...
	this.showRating = function(rating)
	{
		...
		if (span)
		{
			for (var i = 1; i <= rating; i++)
			{
				...
				{
					span.childNodes[i - 1].src = 'img/icons/ratingS.png';
				}
			}
			for (var i = rating + 1; i <= 5; i++)
			{
				if (span.childNodes[i - 1])
				{
					span.childNodes[i - 1].src = 'img/icons/noRatingS.png';
				}
			}
		}
	}
	...
	this.submitRating = function(rating)
	{
		...
		if(element)
		{
			...
			if (select) select.selectedIndex = rating - 1;	
			element.form.submit();
		}
	}	
	this.init();
